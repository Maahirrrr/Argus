    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 animate-fade-in text-[#F5F5F0]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#1D1D1D]">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-sans font-medium bg-[#0066FF] hover:bg-[#0052CC] text-white rounded-[4px] transition-colors cursor-pointer"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-[#0A0A0A] border border-[#1D1D1D] focus:border-[#0066FF] pl-9 pr-3 py-2 text-xs font-sans text-[#F5F5F0] rounded-[4px] outline-none"
          />
        </div>

        <div className="flex items-center bg-[#0A0A0A] p-1 rounded-[4px] border border-[#1D1D1D]">
          {categories.map((c) => (
            <button
              key={c}
              onClick={() => setSelectedCategory(c)}
              className={`px-2.5 py-1 text-[11px] font-sans rounded-[4px] transition-colors cursor-pointer ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5 space-y-2.5">
          {filteredDecisions.map((d) => (
            <div
              ...
              className={`p-3.5 bg-[#0A0A0A] border rounded-[4px] cursor-pointer transition-all ${
                selectedDecision?.id === d.id ? 'border-[#0066FF] bg-[#0066FF]/5' : 'border-[#1D1D1D] hover:border-[#333]'
              }`}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-mono text-[#0066FF]">{d.category || 'Architecture'}</span>
                <span className="text-[10px] font-mono text-[#8A8A8A]">{d.date}</span>
              </div>
              <h3 className="text-xs font-bold text-[#F5F5F0] font-display">{d.title}</h3>
              <p className="text-[11px] font-sans text-[#8A8A8A] mt-1 line-clamp-2">{d.context}</p>
        ...
        {selectedDecision && (
          <div className="lg:col-span-7 bg-[#0A0A0A] border border-[#1D1D1D] rounded-[8px] p-5 space-y-4">
            <div className="border-b border-[#1D1D1D] pb-3">
              <div className="flex items-center gap-2 text-[10px] font-mono text-[#0066FF]">
                ...
                <span className="text-[#1D1D1D]">/</span>
                <span className="text-[#8A8A8A]">{selectedDecision.date}</span>
              </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <div className="text-[10px] font-sans text-[#8A8A8A] uppercase mb-1">Context & Problem Statement</div>
                <p className="text-[#CCCCCC] leading-relaxed font-sans">{selectedDecision.context}</p>
              </div>

              <div>
                <div className="text-[10px] font-sans text-[#8A8A8A] uppercase mb-1">Options Evaluated</div>
                <div className="space-y-1">
                  {optionsList.map((opt, idx) => (
                    <div key={idx} className="p-2 bg-[#050505] border border-[#141414] rounded-[4px] text-[#F5F5F0] font-mono text-[11px]">
                      {idx + 1}. {opt}
                    </div>
                  ))}
                </div>
              ...
              <div>
                <div className="text-[10px] font-sans text-[#00FF88] uppercase mb-1">Winning Consensus Decision</div>
                <div className="p-2.5 bg-[#00FF88]/10 border border-[#00FF88]/30 rounded-[4px] text-[#00FF88] font-mono font-bold text-xs">
                  {selectedDecision.decision || selectedDecision.chosenOption}
                </div>
              </div>

              <div>
                <div className="text-[10px] font-sans text-[#8A8A8A] uppercase mb-1">Strategic Rationale</div>
                <p className="text-[#CCCCCC] leading-relaxed font-sans">{selectedDecision.rationale}</p>
              </div>

              <div>
                <div className="text-[10px] font-sans text-[#8A8A8A] uppercase mb-1">Sign-offs & Approvals</div>
                <div className="flex flex-wrap gap-2 pt-1">
                  {signoffsList.map((s, idx) => (
                    <span key={idx} className="text-[10px] font-mono px-2 py-0.5 bg-[#141414] border border-[#222] text-[#F5F5F0] rounded-[4px]">
                      ✓ {s}
                    </span>
                  ))}
                </div>
